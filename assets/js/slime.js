...
const backgroundRGB = [230, 201, 161];
...
// Create buffer canvas for efficient drawing
...
function update() {
  ...
  for (let i = 0; i < m; i++) {
    for (let j = 0; j < n; j++) {
      // 230 instead of 255 makes them a little less dark
      const intensity = -Math.min(230, Math.floor(trailMap[i][j] * 255));
      const imageJ = n - 1 - j; // Flip vertical axis
      const idx = (imageJ * m + i) * 4;
      data[idx] = Math.max(intensity + backgroundRGB[0], 0); // R
      data[idx + 1] = Math.max(intensity + backgroundRGB[1], 0); // G
      data[idx + 2] = Math.max(intensity + backgroundRGB[2], 0); // B
      data[idx + 3] = 255; // A
    }
  }
